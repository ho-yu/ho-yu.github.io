...
- **형식 없이 자유롭게 써도 된다.** Category, Tag, 제목, 절 번호, Markdown 없이 메모하듯
  작성해도 문맥에 맞춰 노션 스타일의 절·번호·목록·표·강조 박스로 정리한다.
- **내용은 절대 지어내지 않는다.** 사용자가 쓴 내용 안에서만 문장을 다듬고, 요약하고,
  핵심을 강조한다. 작성하지 않은 지식·수치·인과관계·경험·코드·실행 결과·수식·예시는
  추가하지 않는다. `같다`, `듯하다` 같은 불확실한 표현도 확정된 사실로 바꾸지 않는다.
  내용이 없거나 서로 모순되는 부분은 본문에 임의로 채우지 않고
...
관련 글도 직접 찾거나 링크할 필요가 없다. 공통 Tag와 2차 Category를 이용한 하단 카드는
빌드할 때 자동으로 만들어진다. Tag 수 제한으로 놓칠 수 있는 의미상 관련 글은
`작성완료` 처리 중 본문 최하단의 **관련 글** 절에 게시물 제목 링크로 자동 연결한다.
관련성이 명확한 글이 없으면 억지로 표시하지 않는다.
...
`published: false`를 제거하고 빌드를 검사한다.

발행 후에도 `workspace/` 안의 파일은 손대지 않는다. 사용한 작업 슬롯을 초기화하거나
정리하지 않으며, `workspace/`에 대한 수정·commit·push는 사용자가 별도로 명시적으로
요청할 때만 한다.
