...
- **train/valid/test 역할** : train=학습, valid=반복 비교·선택, test=단 1회 최종 확인
- **test_size** : 떼어낼 비율 / **stratify** : 클래스 비율 유지 (분류 전용)
- **회귀 지표** : MAE(이상치 둔감) · RMSE(큰 오차 민감) · R²(음수 가능, 0=평균 기준)
- **분류 지표** : Precision(FP 비용) · Recall(FN 비용) · F1(균형)
- **Accuracy 함정** : 불균형 데이터에서 다수 클래스만 맞혀도 높게 나옴
- **test 재사용 금지** : 반복 확인하면 사실상 학습에 쓰인 것 → 성능 부풀려짐
