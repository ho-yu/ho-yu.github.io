...
- **`핵심 기억 카드`(기억할 내용)는 문장형 서술이 아니라 핵심 키워드 중심의 간결한
  목록으로 정리한다.** `**키워드** : 한 줄 설명` 형태를 기본으로 하고, Q&A 형태로
  쓰여 있었더라도 질문·답변을 키워드-설명 쌍으로 압축한다. 원문의 의미와 사실 관계는
  그대로 유지한 채 표현만 압축한다 — 압축 과정에서 새로운 사실을 추가하지 않는다.
